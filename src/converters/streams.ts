...
export type NormalizedStreamEvent =
  ...
  | { type: "end"; finishReason: string | null; usage?: import("./shared.js").NormalizedUsage };
  ...
  parse(event: ResponseStreamEvent): NormalizedStreamEvent[] {
    ...
    switch (event.type) {
      ...
      case "response.reasoning_summary_part.done": {
        const key = `thinking_${event.output_index}_${event.summary_index}`;
        const idx = this.blockMapping.get(key);
        if (idx != null && event.type === "response.reasoning_summary_part.done") {
          out.push({ type: "content_done", index: idx });
        }
        break;
      }
    ...
    switch (event.type) {
      ...
      case "message_delta": {
        out.push({
          ...
          usage: normalizeUsage(event.usage as Record<string, unknown> | undefined),
        });
        break;
      }
    }
...
export class OpenAIChatStreamEmitter implements StreamEmitter {
  ...
  private chunk(delta: Record<string, unknown>, finishReason: string | null = null, usage?: import("./shared.js").NormalizedUsage): ChatCompletionChunk {
    return {
      ...
      ...(usage ? { usage: denormalizeUsageToOpenAIChat(usage) } : {}),
...
export class ResponsesStreamEmitter implements StreamEmitter {
  ...
  private buildResponseShell(status: string, usage?: import("./shared.js").NormalizedUsage): Record<string, unknown> {
    return {
      id: this.id,
      object: "response",
    ...
    switch (event.type) {
      ...
      case "end": {
        out.push({
          ...
          usage: denormalizeUsageToAnthropic(event.usage) ?? { output_tokens: 0 },
        });
