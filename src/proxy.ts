...
import { writeGatewayLog } from "./file-log.js";
...
async function upstreamFetch(config: ModelConfig, body: string, stream: boolean, options?: UpstreamRequestOptions): Promise<Response> {
  ...
  writeGatewayLog("upstream.request", {
  ...
  if (!res.ok) {
    ...
    writeGatewayLog("upstream.response.error", {
      target_model: config.name,
      provider: config.provider,
      upstream_model: config.model,
      url,
  ...
  writeGatewayLog("upstream.response.meta", {
    target_model: config.name,
    provider: config.provider,
    upstream_model: config.model,
    url,
...
export async function passthroughRequest(
  ...
  writeGatewayLog("upstream.response.body", {
    target_model: config.name,
    provider: config.provider,
    upstream_model: config.model,
    stream: false,
