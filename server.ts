...
import { getGatewayLogPath, writeGatewayLog } from "./src/file-log.js";
...
async function executeModelRequest(
  ...
  writeGatewayLog("request.body.normalized", {
    incoming_format: incomingFormat,
    target_provider: modelConfig.provider,
    requested_model: rawBody.model,
    target_model: modelConfig.name,
    stream,
    body: normalized,
  });
...
function createRoute(incomingFormat: StreamFormat) {
  return async (c) => {
    ...
    writeGatewayLog("request.body.in", {
      path: c.req.path,
      incoming_format: incomingFormat,
...
const server = serve({ fetch: app.fetch, port: config.port }, (info) => {
  ...
  console.log(`Log file (hourly JSONL): ${getGatewayLogPath()}`);
